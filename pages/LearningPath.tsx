    ...
    const handleGeneratePath = async () => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                ...
                contents: `Given the following knowledge graph nodes, arrange them into a logical, linear learning curriculum. 
            ...
            if (response.text) {
                const pathData = JSON.parse(response.text) as LearningPathData;
                
                // Force first module to be active
                            ...
                            <div key={idx} className="relative pl-16">
                                ...
                                <div className={`absolute left-0 top-0 size-12 rounded-full border-4 flex items-center justify-center bg-white z-10 
                                    ${module.status === 'completed' ? 'border-emerald-500 text-emerald-500' : 
                                      module.status === 'active' ? 'border-primary text-primary shadow-lg shadow-primary/20' : 
                                      'border-slate-300 text-slate-300'}`}
                                >
                                    <span className="font-bold text-lg">{idx + 1}</span>
                                </div>

                                <div className={`bg-white rounded-2xl p-6 border transition-all ${module.status === 'active' ? 'border-primary shadow-md ring-1 ring-primary/20' : 'border-slate-200 shadow-sm opacity-90'}`}>
                                        ...
                                        {module.nodeIds.map(nodeId => {
                                            ...
                                            return node ? (
                                                <button 
                                                    ...
                                                    ${module.status === 'locked' ? 'bg-slate-50 border-slate-100 text-slate-400 cursor-not-allowed' : btnClass}`}
                                                >
                                                    {status === 'none' && <span className={`size-2 rounded-full ${node.type==='concept'?'bg-primary':node.type==='fact'?'bg-emerald-400':'bg-amber-400'}`}></span>}
